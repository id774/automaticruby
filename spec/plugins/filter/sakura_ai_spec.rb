...
require 'filter/sakura_ai'
require 'json'
require 'net/http'

# FilterSakuraAI needs a Sakura AI Engine token; doc/PLUGINS.md section 6.3
...
# each answer means -- and no example reaches the service. Only the Sakura AI
# Engine is exercised: the other AI filters have their own specs, and none of
...
describe Automatic::Plugin::FilterSakuraAI do
  let(:settings) {
    { 'token' => 'test-token', 'model' => 'sakura-test', 'prompt' => 'Summarize this.',
      'retry' => 1, 'interval' => 0 }
  }
  ...
  def run(config, pipeline)
    Automatic::Plugin::FilterSakuraAI.new(config, pipeline).run
  end

  describe 'the request it builds' do
    before {
      serve(SakuraAISpec.answered('a summary'))
      run(settings, one_item)
    }

    it "posts to the Sakura AI Engine's own endpoint, over verified TLS" do
      connections[0][0].should == 'api.ai.sakura.ad.jp'
  ...
  describe 'what it does to the pipeline' do
    ...
    it 'asks once for each item' do
      serve(SakuraAISpec.answered('first summary'), SakuraAISpec.answered('second summary'))
  ...
  describe 'when the request fails' do
    it 'retries a server error and carries on' do
      serve(SakuraAISpec.refused(Net::HTTPInternalServerError, '500', 'server error'),
            SakuraAISpec.answered('a summary'))
      returned = run(settings, one_item)
    ...
    it 'retries a rate limit' do
      serve(SakuraAISpec.refused(Net::HTTPTooManyRequests, '429', 'too many requests'),
            SakuraAISpec.answered('a summary'))
    ...
    it 'gives up after the configured number of retries' do
      serve(SakuraAISpec.refused(Net::HTTPInternalServerError, '500', 'server error'),
            SakuraAISpec.refused(Net::HTTPInternalServerError, '500', 'server error'))
    ...
    it 'does not retry a rejected request' do
      serve(SakuraAISpec.refused(Net::HTTPUnauthorized, '401', 'invalid token'))
      lambda { run(settings, one_item) }.
        should raise_error(Automatic::Plugin::FilterSakuraAI::Error, /401/)
      posted.should have(1).request
    end
  ...
  describe 'when the answer cannot be read' do
    it 'raises on a body that is not JSON' do
      serve(SakuraAISpec.response(Net::HTTPOK, '200', 'not json at all'))
      lambda { run(settings, one_item) }.
        should raise_error(Automatic::Plugin::FilterSakuraAI::Error, /not JSON/)
      posted.should have(1).request
    end

    it 'raises on a body without the choice it expects' do
      serve(SakuraAISpec.response(Net::HTTPOK, '200', JSON.generate('id' => 'cmpl_1')))
      lambda { run(settings, one_item) }.
    ...
    it 'raises rather than writing an empty description' do
      serve(SakuraAISpec.response(Net::HTTPOK, '200', JSON.generate(
        'choices' => [{ 'message' => { 'role' => 'assistant', 'content' => '' } }]
      )))
      pipeline = one_item
      lambda { run(settings, pipeline) }.
        should raise_error(Automatic::Plugin::FilterSakuraAI::Error, /no content/)
      pipeline[0].items[0].description.should == 'the body of A'
    end
  end
